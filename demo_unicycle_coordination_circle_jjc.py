...
desired_inter_vehicle_theta = (2*np.pi/num_of_agents)*\
np.ones((len(list_of_edges),1))
...
for i in range(num_of_agents):
    ...
    list_of_agents.append(ag.AgentUnicycle(\
    (107, 155, 200, 0),\
    i, 1000*np.random.rand(2,1),\
    60*np.array([[np.cos(theta_o[0])],[np.sin(theta_o[0])]]))) #60
    #list(pygame.colordict.THECOLORS.items())[np.random.randint(0,657)][1]
for agent in list_of_agents:
    agent.traj_draw = True
    agent.wrt_label = True
# GVF
ke_circle = 5e-5
kd_circle = 60
...
yo = 500
ro = 150 # radius
direction = 1 # turning direction: clockwise=1 counterclockwise = -1
# run simulation
pygame.init()
...
while(runsim):
    ...
    pygame.draw.circle(screen,(255,0,0), (xo,yo), 150, 1)  #drawing the Circle
    ...
    dr = -direction*k_coord*B_dir.dot(error_theta)
    
    # Guiding vector field
    for idx,agent in enumerate(list_of_agents):
        agent.draw(screen)
        circle_path = gvf.Path_gvf_circle(xo, yo, ro+dr[idx])
        ut = gvf.gvf_control_2D_unicycle(agent.pos, agent.vel, ke_circle,\
        kd_circle, circle_path, direction)
